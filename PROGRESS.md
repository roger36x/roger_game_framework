...
### NLua Lua 5.4 类型兼容修复（2026-02-28）
- **问题**：Lua 5.4 整数字面量为 `long` 类型，NLua 无法自动匹配到 C# `double` 参数；`spawn()` 省略第 4 参数时参数个数不匹配
- **修复**：GameAPI.Register() 中用 Lua shim 函数包装 `spawn` 和 `add_light`，在 Lua 侧 `* 1.0` 强制整数转浮点，同时处理可选参数
- **降低环境音量**：AmbientVolume 从 0.3f 降至 0.1f，天气音效更隐约

## 待完成里程碑

| 里程碑 | 目标 | 状态 |
|--------|------|------|
| Milestone 9 | 网络与多人 | 搁置 |
